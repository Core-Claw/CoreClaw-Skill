...
一个面向 AI coding agent 的可复用 CoreClaw 工作流定义。

## 项目概览

CoreClaw 通过 REST API 提供 scraper 的搜索与执行能力。这个仓库围绕 `SKILL.md` 和 `openapi.json` 对标准 CoreClaw 工作流进行了 Skill 化整理。
...
- 在 CoreClaw Store 中找到合适的 scraper
...
└── openapi.json      # CoreClaw OpenAPI 规范
```

## 当前能力

当前 Skill 已覆盖 CoreClaw 的核心工作流：

- 搜索 CoreClaw Store 中可用的 scraper
...
- 当 Skill 被触发后，Agent 会按文档中的流程一步步完成 CoreClaw 工作流
...
- `curl`
- `jq`
- 有效的 `CORECLAW_API_KEY`
- 可以访问 `https://openapi.coreclaw.com`
...
- CoreClaw OpenAPI 规范：`openapi.json`
- Skill 入口定义：`SKILL.md`
...
本仓库为 CoreClaw 数据采集流程接入 AI Agent 环境提供了结构化的起点。
